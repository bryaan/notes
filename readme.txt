...
KServer Containers
- prob running coreos
- phusion/baseimage -> virtualbox -> win 10 SPCS + VNC into *windows* (so no need to mess with gnome-session)
	- vbox image should be pull from a precreated image with everything setup.
	- cron job for image snapshots. mount volume to snapshot storage.
